...
public class PlaylistDownloader {
    ...
    public void downloadToFile(@NotNull String outfile) throws IOException {
        downloadToFile(outfile, null);
    }

    public void downloadToFile(@NotNull String outfile, @Nullable String key) throws IOException {
        ByteArrayOutputStream outputStream = download(key);
        try(OutputStream fileOutputStream = new FileOutputStream(outfile)) {
            outputStream.writeTo(fileOutputStream);
        }
    }

    public ByteArrayOutputStream download(@Nullable String key) throws IOException {
        fetchPlaylist();

        ByteArrayOutputStream outputStream = new ByteArrayOutputStream(); // TODO: think about better way
        ...
        for (String line : playlist) {
            ...
            if (line.startsWith(EXT_X_KEY)) {
                ...
                downloadInternal(segmentUrl, outputStream);
            }
        }
        return outputStream;
    }
    ...
    @NotNull
    private OutputStream openStream(@NotNull String filePath) throws IOException {
        File file = new File(filePath);
        return new FileOutputStream(file, file.exists());
    }

    private void downloadInternal(URL segmentUrl, OutputStream out) throws IOException {
    ...
    private OutputStream openOutputStream(URL segmentUrl, String outFile) throws IOException {
